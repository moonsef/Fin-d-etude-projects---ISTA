...
public class LoginModal extends DialogFragment {
    ...
    public Dialog onCreateDialog(Bundle savedInstanceState) {
        ...
        v.findViewById(R.id.login).setOnClickListener(l -> loginClient(dialog, v));
        v.findViewById(R.id.d).setOnClickListener(l -> {
            dialog.cancel();
            DialogFragment fragment = new RegisterModal();
            fragment.show(getParentFragmentManager(), "modal1");
        });
        return dialog;
    }

    private void loginClient(AlertDialog dialog, View v) {
        ...
        try {
            ...
            retrofit.create(ClientAPI.class).loginClient(user).enqueue(new Callback<String>() {
                @Override
